...
export async function POST(request: NextRequest) {
  try {
      ...
      return NextResponse.json(
        {
          success: false,
          error:
            "Missing required fields: userId, testId, testType, questionNumbers",
        },
        { status: 400 }
      );
    }
    ...
    for (const questionNumber of body.questionNumbers) {
      ...
      const speakingAnswer = await SubmitSpeakingAnswerModel.create(
        submissionData
      );
...
export async function GET(request: NextRequest) {
  try {
    ...
    const speakingAnswers = await SubmitSpeakingAnswerModel.find(query).sort({
      submittedAt: -1,
    });
